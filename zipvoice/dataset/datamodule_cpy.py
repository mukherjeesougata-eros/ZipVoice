...
class TtsDataModule:
    ...
    def dev_dataloaders(self, cuts_valid: CutSet, rank: int = 0, world_size: int = 1) -> DataLoader:
        ...
        dev_sampler = DynamicBucketingSampler(
            ...
            max_duration=self.args.max_duration,
            shuffle=False,
            rank=rank,          # ← each rank sees only its slice
            world_size=world_size,  # ← total GPU count
            drop_last=True,     # ← ensures equal batches across all ranks
        )

...
from zipvoice.dataset.datamodule_cpy import TtsDataModule
...
def get_parser():
    ...
    #     type=int,
    #     default=0,
    #     help="""Resume training from this iteration. It should be positive.
    #     If larger than 0, it will load checkpoint from
    #     exp-dir/checkpoint-{start_iter}.pt
    #     """,
    # )
...
#    skip_batches = params.batch_idx_train % len(train_dl)
    ...
    for batch_idx, batch in enumerate(train_dl):
        ...
        #print(f"Inside loop for 1st time: params.batch_idx_train = {params.batch_idx_train}")

        if batch_idx % 10 == 0:
            set_batch_count(model, get_adjusted_batch_count(params) + 100000)

        #print(f"Inside loop for 2nd time: params.batch_idx_train = {params.batch_idx_train}")
...
def compute_validation_loss(
    ...
    #print("Inside compute_validation_loss")
    #rank = model.device.index  # gets the GPU index this rank is using
    #print(f"[Rank {rank}] Starting validation", flush=True)
    try:
        model.eval()
        device = model.device if isinstance(model, DDP) else next(model.parameters()).device
        ...
        for batch_idx, batch in enumerate(valid_dl):
            #print(f"[Rank {rank}] Processing validation batch {batch_idx}", flush=True)
            tokens, features, features_lens = prepare_input(
                params=params,
                batch=batch,
                device=device,
        ...
        import traceback
        #print(f"[Rank {rank}] EXCEPTION in compute_validation_loss:\n{traceback.format_exc()}", flush=True)
        raise  # re-raise so the process exits cleanly
...
def resume_latest_checkpoint(
    ...
    It will load the checkpoint from
    `params.exp_dir/checkpoint-{params.batch_idx_train}.pt`.
    ...
    #print(f"Inside resume_latest_checkpoint: filename = {filename}")
...
def run(rank, world_size, args):
    ...
    #    print(f"Rank {rank} param {name} shape {tuple(p.shape)}", flush=True)
    ...
    if params.start_epoch > 1:
        ...
        #print(f"params.exp_dir inside run = {params.exp_dir}")        
        latest_ckpt = find_latest_checkpoint(params.exp_dir)
        if latest_ckpt is not None:
            logging.info(f"Resuming from checkpoint {latest_ckpt}")
            latest_ckpt = Path(latest_ckpt)
            if params.distill_stage == "first":
                checkpoints = resume_latest_checkpoint(
    ...
    # checksum = torch.tensor([sum(p.float().abs().sum() for p in model.parameters())], device=f"cuda:{rank}")
    # dist.all_reduce(checksum)
    # print(f"Rank {rank} checksum:", checksum.item())
    #print(f"Rank {rank} using device {torch.cuda.current_device()}", flush=True)
    if world_size > 1:
    ...
    elif latest_ckpt is not None and checkpoints is not None:
        # load state_dict for optimizers
        if "optimizer" in checkpoints:
            logging.info("Loading optimizer state dict")
            optimizer.load_state_dict(checkpoints["optimizer"])
    ...
    #print(f"After loading checkpoints: checkpoints['batch_idx_train'] = {checkpoints['batch_idx_train']} and params.batch_idx_train = {params.batch_idx_train}")               
    ...
    valid_dl = datamodule.dev_dataloaders(dev_cuts, rank=rank, world_size=world_size)

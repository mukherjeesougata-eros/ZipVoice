...
def load_model(model_path):
    #model_path = os.path.join(model_dir, "wer/indic-/")
    if not os.path.exists(model_path):
        logging.error(
            f"Error: Indic-conformer model not found at {model_path}. "
            "Please download evaluation modelss from "
            "https://huggingface.co/k2-fsa/TTS_eval_models "
            "and pass this directory with --model-dir."
        )
        exit(1)
    logging.info(f"Loading Indic-conformer model from: {model_path}")

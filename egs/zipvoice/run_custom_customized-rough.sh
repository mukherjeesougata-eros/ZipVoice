...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
      ...
      for subset in train dev;do
            python3 -m zipvoice.bin.prepare_dataset \
                  --tsv-path data_IndicTTSpunjabi/raw/custom_${subset}.tsv \
                  --prefix custom \
                  --subset ${subset} \
                  --num-jobs ${nj} \
                  --output-dir data_IndicTTSpunjabi/manifests
      done
...
if [ $stage -le 2 ] && [ $stop_stage -ge 2 ]; then
      ...
      for subset in train dev; do
            python3 -m zipvoice.bin.compute_fbank \
                  --source-dir data_IndicTTSpunjabi/manifests \
                  --dest-dir data_IndicTTSpunjabi/fbank \
                  --dataset custom \
                  --subset ${subset} \
                  --num-jobs ${nj}
      done
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
      ...
      python3 ./local/prepare_token_file_char.py \
            --manifest data_IndicTTSpunjabi/manifests/custom_cuts_train.jsonl.gz \
            --tokens data_IndicTTSpunjabi/tokens_custom.txt
fi
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
      ...
      python3 -m zipvoice.bin.train_zipvoice \
            ...
            --exp-dir exp/zipvoice_custom/default_setup 
fi

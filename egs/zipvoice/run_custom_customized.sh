...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
      ...
      for subset in train dev;do
            python3 -m zipvoice.bin.prepare_dataset \
                  --tsv-path data_l/raw/custom_${subset}.tsv \
                  --prefix custom \
                  --subset ${subset} \
                  --num-jobs ${nj} \
                  --output-dir data_l/manifests
      done
...
if [ $stage -le 2 ] && [ $stop_stage -ge 2 ]; then
      ...
      for subset in train dev; do
            python3 -m zipvoice.bin.compute_fbank \
                  --source-dir data_l/manifests \
                  --dest-dir data_l/fbank \
                  --dataset custom \
                  --subset ${subset} \
                  --num-jobs ${nj}
      done
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
      ...
      python3 ./local/prepare_token_file_char.py \
            --manifest data_l/manifests/custom_cuts_train.jsonl.gz \
            --tokens data_l/tokens_custom.txt
fi
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
      ...
      python3 -m zipvoice.bin.train_zipvoice \
            ...
            --tokenizer simple \
            --token-file data_l/tokens_custom.txt \
            --dataset custom \
            --train-manifest data_l/fbank/custom_cuts_train.jsonl.gz \
            --dev-manifest data_l/fbank/custom_cuts_dev.jsonl.gz \
            --exp-dir exp/zipvoice_custom/60-epochs 
fi
...
if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then
      ...
      python3 -m zipvoice.bin.infer_zipvoice \
            --model-name zipvoice \
            --model-dir exp/zipvoice_custom/default_setup \
            --checkpoint-name iter-60000-avg-2.pt \
            --tokenizer simple \
            --test-list data/Test/test_all_lang.tsv \
            --res-dir results/test_custom/ZipVoice/all_lang
fi
